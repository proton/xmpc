...
xfmpc_song_dialog_get_type (void)
{
	...
	if (G_UNLIKELY (xfmpc_song_dialog_type == G_TYPE_INVALID))
	{
		...
		xfmpc_song_dialog_type = g_type_register_static (GTK_TYPE_DIALOG, "XfmpcSongDialog", &xfmpc_song_dialog_info, 0);
	}
																...
																XfmpcSongInfo *song_info)
{
	...
	gtk_widget_set_visible (priv->hbox_artist, song_info->artist!=NULL);
	gtk_widget_set_visible (priv->hbox_album, song_info->album!=NULL);
	gtk_widget_set_visible (priv->hbox_title, song_info->title!=NULL);
	gtk_widget_set_visible (priv->hbox_date, song_info->date!=NULL);
	gtk_widget_set_visible (priv->hbox_track, song_info->track!=NULL);
	gtk_widget_set_visible (priv->hbox_genre, song_info->genre!=NULL);
