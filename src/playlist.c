...
static void             cb_playlist_changed                     (XfmpcPlaylist *playlist);
static void             cb_random                               (XfmpcPlaylist *playlist,
																																gboolean random);
static void             cb_repeat                               (XfmpcPlaylist *playlist,
																																gboolean repeat);
...
void                    cb_save_selection                       (XfmpcPlaylist *playlist);
void                    cb_info_selection                       (XfmpcPlaylist *playlist);
static void             cb_repeat_switch                        (XfmpcPlaylist *playlist);
static void             cb_random_switch                        (XfmpcPlaylist *playlist);
...
xfmpc_playlist_init (XfmpcPlaylist *playlist)
{
	...
	mi = gtk_menu_item_new_with_label (_("Save playlist"));
	gtk_menu_shell_append (GTK_MENU_SHELL (priv->menu), mi);
	g_signal_connect_swapped (mi, "activate",
														G_CALLBACK (cb_save_selection), playlist);
	...
	g_signal_connect_swapped (priv->menu_item_repeat, "activate",
														G_CALLBACK (cb_repeat_switch), playlist);
	...
	g_signal_connect_swapped (priv->menu_item_random, "activate",
														G_CALLBACK (cb_random_switch), playlist);
	...
	/* Menu -> Clear playlist */
	mi = gtk_image_menu_item_new_with_mnemonic (_("Clear Playlist"));
	gtk_menu_shell_append (GTK_MENU_SHELL (priv->menu), mi);
	g_signal_connect_swapped (mi, "activate",
														G_CALLBACK (xfmpc_mpdclient_playlist_clear), playlist->mpdclient);
	...
	g_signal_connect_swapped (playlist->mpdclient, "repeat",
														G_CALLBACK (cb_repeat), playlist);
	g_signal_connect_swapped (playlist->mpdclient, "random",
														G_CALLBACK (cb_random), playlist);
	/* Tree view */
...
static void
cb_random (XfmpcPlaylist *playlist, gboolean random)
{
	XfmpcPlaylistPrivate *priv = XFMPC_PLAYLIST (playlist)->priv;
	GTK_CHECK_MENU_ITEM(priv->menu_item_random)->active = random;
}
static void
cb_repeat (XfmpcPlaylist *playlist, gboolean repeat)
{
	XfmpcPlaylistPrivate *priv = XFMPC_PLAYLIST (playlist)->priv;
...
void
cb_save_selection (XfmpcPlaylist *playlist)
{
	//XfmpcPlaylistPrivate   *priv = XFMPC_PLAYLIST (playlist)->priv;
	...
	if (response == GTK_RESPONSE_OK)
	{
		...
		while (NULL != list)
		{
			if (gtk_tree_model_get_iter (store, &iter, list->data))
			{
				gtk_tree_model_get (store, &iter, COLUMN_FILENAME, &filename, -1);
				xfmpc_mpdclient_database_playlist_append
					(playlist->mpdclient, gtk_entry_get_text (GTK_ENTRY (entry)), filename);
				g_free(filename);
			}
			list = g_list_next (list);
		}
...
static void
cb_repeat_switch (XfmpcPlaylist *playlist)
{
	xfmpc_mpdclient_set_repeat (playlist->mpdclient,
															!xfmpc_mpdclient_get_repeat (playlist->mpdclient));
}

static void
cb_random_switch (XfmpcPlaylist *playlist)
{
	xfmpc_mpdclient_set_random (playlist->mpdclient,
															!xfmpc_mpdclient_get_random (playlist->mpdclient));
}
