...
static void             cb_cover_changed                        (XfmpcInterface *interface);
...
xfmpc_interface_init (XfmpcInterface *interface)
{
	...
	interface->cover = xfmpc_ssignal_get ();
	...
	/* Cover */
	priv->cover_img = gtk_image_new_from_file (NULL);
	...
	//label = priv->subtitle = gtk_label_new (PACKAGE_STRING); //TODO:uncomment
	gtk_label_set_attributes (GTK_LABEL (label), attrs);
	gtk_label_set_selectable (GTK_LABEL (label), TRUE);
	gtk_label_set_ellipsize (GTK_LABEL (label), PANGO_ELLIPSIZE_END);
	gtk_misc_set_alignment (GTK_MISC (label), 0, 0.5);
	...
	gtk_box_pack_start (GTK_BOX (hbox), priv->button_prev, FALSE, FALSE, 0);
	gtk_box_pack_start (GTK_BOX (hbox), priv->button_pp, FALSE, FALSE, 0);
	gtk_box_pack_start (GTK_BOX (hbox), priv->button_next, FALSE, FALSE, 0);
	gtk_box_pack_start (GTK_BOX (hbox), progress_box, TRUE, TRUE, BORDER);
	gtk_box_pack_start (GTK_BOX (hbox), priv->button_volume, FALSE, FALSE, 0);
...
//	gtk_container_add (GTK_CONTAINER (vbox), priv->subtitle); //TODO:
	...
	gtk_box_pack_start (GTK_BOX (ibox), vbox, FALSE, FALSE, 0);

	GtkWidget *box = gtk_hbox_new (FALSE, 0);
	gtk_box_pack_start (GTK_BOX (interface), box, FALSE, FALSE, 0);
	gtk_box_pack_start (GTK_BOX (box), priv->cover_img, FALSE, FALSE, 3);
	gtk_box_pack_start (GTK_BOX (box), ibox, TRUE, TRUE, 5);
	...
	g_signal_connect_swapped (interface->cover, "cover-update",
														G_CALLBACK (cb_cover_changed), interface);
