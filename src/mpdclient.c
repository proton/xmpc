...
const gchar *
xfmpc_mpdclient_get_file (XfmpcMpdclient *mpdclient)
{
	XfmpcMpdclientPrivate *priv = XFMPC_MPDCLIENT (mpdclient)->priv;
	mpd_Song *song = mpd_playlist_get_current_song (priv->mi);
	if(G_UNLIKELY (NULL == song)) return NULL;
	return song->file;
}
															...
															gchar **length)
{
	...
	if (NULL == data) data = mpd_playlist_get_changes (priv->mi, -1);
	else data = mpd_data_get_next (data);

	if (NULL != data)
	{
		if (NULL != filename) *filename = g_strdup (data->song->file);
		if (NULL != song) *song = _get_formatted_name (data->song);
		if (NULL != length) *length = g_strdup_printf ("%d:%02d", data->song->time / 60, data->song->time % 60);
		if (NULL != pos) *pos = data->song->pos;
		*id = data->song->id;
	}
...
void
xfmpc_mpdclient_playlist_end (XfmpcMpdclient *mpdclient)
{
	//XfmpcMpdclientPrivate *priv = mpdclient->priv;
	//MpdData               *data;
	gchar                 *basename;
...
gboolean
xfmpc_mpdclient_database_playlist_read_contents (XfmpcMpdclient *mpdclient,
															...
															gchar **filename)
{
	...
	if (NULL == data) data = mpd_database_get_playlist_content (priv->mi, playlist);
	else data = mpd_data_get_next (data);

	if (NULL != data && data->type == MPD_DATA_TYPE_SONG)
		*filename = g_strdup (data->song->file);
...
gboolean
xfmpc_mpdclient_database_playlist_read (XfmpcMpdclient *mpdclient,
															gchar **name)
{
	XfmpcMpdclientPrivate *priv = XFMPC_MPDCLIENT (mpdclient)->priv;
	static MpdData        *data = NULL;

	if (NULL == data) data = mpd_database_playlist_list (priv->mi);
	else data = mpd_data_get_next (data);

	if (NULL != data && data->type == MPD_DATA_TYPE_PLAYLIST)
		*name = g_strdup (data->playlist->path);

	return NULL != data;
}

void
xfmpc_mpdclient_database_playlist_create (XfmpcMpdclient *mpdclient,
													const gchar *name)
{
	XfmpcMpdclientPrivate *priv = XFMPC_MPDCLIENT (mpdclient)->priv;
	mpd_database_playlist_clear (priv->mi, name);
}

void
xfmpc_mpdclient_database_playlist_rename (XfmpcMpdclient *mpdclient,
													const gchar *old_name,
													const gchar *name)
{
	XfmpcMpdclientPrivate *priv = XFMPC_MPDCLIENT (mpdclient)->priv;
	mpd_database_playlist_rename (priv->mi, old_name, name);
}

void
xfmpc_mpdclient_database_playlist_remove (XfmpcMpdclient *mpdclient,
													const gchar *name)
{
	XfmpcMpdclientPrivate *priv = XFMPC_MPDCLIENT (mpdclient)->priv;
	mpd_database_delete_playlist (priv->mi, name);
}

void
xfmpc_mpdclient_database_playlist_append (XfmpcMpdclient *mpdclient,
													const gchar *playlist,
													const gchar *filename)
{
	XfmpcMpdclientPrivate *priv = XFMPC_MPDCLIENT (mpdclient)->priv;
	mpd_database_playlist_list_add  (priv->mi, playlist, filename);
}
...
XfmpcSongInfo *
xfmpc_mpdclient_get_current_song_info (XfmpcMpdclient *mpdclient)
{
	XfmpcMpdclientPrivate *priv = XFMPC_MPDCLIENT (mpdclient)->priv;
	return xfmpc_mpdclient_get_song_info(mpdclient, mpd_player_get_current_song_id (priv->mi));
}
